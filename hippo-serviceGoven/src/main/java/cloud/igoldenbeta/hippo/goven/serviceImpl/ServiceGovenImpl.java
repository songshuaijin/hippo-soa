...
public class ServiceGovenImpl implements ServiceGovern {
  ...
  public int register(String arg0) {
    ...
    InputStream in = null;
    try {
      File file = new File(ServiceGovenImpl.class.getResource("/config.properties").getPath());
      if (file.exists()) {
      ...
      if (StringUtils.isNotBlank(host)) {
        ...
        if (properties.get("erueka.ipAddress") != null) {
          eureInstanceConfigBean.setIpAddress(properties.getProperty("erueka.ipAddress"));
        } else {
      ...
      if (StringUtils.isNotBlank(port)) {
        ...
        if (properties.getProperty("eureka.port") != null) {
          eureInstanceConfigBean
              .setNonSecurePort(Integer.parseInt(properties.getProperty("eureka.port")));
        } else {
      ...
      serviceUrls.put(EurekaClientConfigBean.DEFAULT_ZONE,
          properties.getProperty("eureka.serviceUrls") == null
